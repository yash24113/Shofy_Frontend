...
const DetailsThumbWrapper = ({
  ...
  const mainImageUrl = useMemo(() => {
    // Prefer externally controlled active image first (so clicks persist)
    if (activeImg) return processImageUrl(activeImg);
    // Fall back to the provided main img
    if (img) return processImageUrl(img);
    // Fall back to the first available image from the thumbnails
  ...
  useEffect(() => {
    console.log('Main Image Debug:', {
      ...
      activeImg,
      mainImageUrl,
      processedImageURLs: processedImageURLs.map(x => ({
        type: x.type,
        img: x.img,
        video: x.video ? 'video-present' : null
      }))
    });
  }, [img, activeImg, mainImageUrl, processedImageURLs]);

  // Update main source when the derived default main image changes
  useEffect(() => {
